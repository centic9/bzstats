...
datam <- melt(data, id.vars="Date", measure.vars=
	# when you change the order here also adjust it below for the legend
	c("Open.overall","Actual.bugs","Workable.bugs","Needinfo","Enhancements","Bugs.with.patch"))
labeldatam <- melt(labeldata, id.vars="Date", measure.vars=c("Open.overall","Actual.bugs","Enhancements",
                                                   "Needinfo","Workable.bugs","Bugs.with.patch"))

library(ggplot2)

ggplot(datam, aes(x=parse_date(Date, approx = TRUE), y=value, colour=variable, label=value)) +
    ...
    geom_text(data = labeldatam, hjust=1, vjust=0, angle = 90) +
